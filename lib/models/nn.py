...
class NN(Configurable):
  ...
  def MRNN(self, inputs, bi_dir=False):#htscore sequence length=1,so no bidirectional
    """"""

    input_size = inputs.get_shape().as_list()[-1]
    output_size = self.mem_size
    cell = self.recur_cell(self._config, out_size=output_size, input_size=input_size, moving_params=self.moving_params)
    lengths = 1
    ...
    top_recur, end_recur = rnn.dynamic_rnn(cell, inputs,
                                           ...
                                           recur_keep_prob=recur_keep_prob,
                                           dtype=tf.float32)
    top_recur.set_shape([tf.Dimension(None), tf.Dimension(None), tf.Dimension(self.mem_size)])
    if self.moving_params is None:
      if self.recur_cell.__name__ != 'GRUCell':
        with tf.variable_scope("%s/MLinear" % (self.recur_cell.__name__)):
          matrix = tf.get_variable('Weights', [input_size, self.mem_size*2], initializer= tf.random_normal_initializer())
          n_splits = matrix.get_shape().as_list()[-1] // self.mem_size
          I = tf.diag(tf.ones([self.mem_size]))
          for W in tf.split(1, n_splits, matrix):
            WTWmI = tf.matmul(W, W, transpose_a=True) - I
            tf.add_to_collection('ortho_losses', tf.nn.l2_loss(WTWmI))
      else:
        for name in ['Gates', 'Candidate']:
          with tf.variable_scope("GRUCell/%s/MLinear" % (name)):
            matrix = tf.get_variable('Weights', [input_size, self.mem_size*2], initializer= tf.random_normal_initializer())
            n_splits = matrix.get_shape().as_list()[-1] // self.mem_size
            I = tf.diag(tf.ones([self.mem_size]))
            for W in tf.split(1, n_splits, matrix):
