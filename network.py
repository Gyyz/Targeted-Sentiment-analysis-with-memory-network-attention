...
class AttWork(Configurable):
    ...
    def __init__(self, model, *args, **kwargs):
        ...
        super(AttWork, self).__init__(*args, **kwargs)
            ...
            while total_pretrain_iters < pretrain_iters:
                for j, (feed_dict, sents) in enumerate(self.train_minibatches()):
                    ...
                    if j % print_every == 0:
                        ...
                        print('%6d) Pretrain loss: %.2e (%.2e + %.2e + %.2e)    Pretrain rate: %6.1f sents/sec' % (
                        total_pretrain_iters, pretrain_loss, recur_loss, covar_loss, ortho_loss, pretrain_time))
                        pretrain_time = 0
                        pretrain_loss = 0
                        pretrain_recur_loss = 0
                        pretrain_covar_loss = 0
        ...
        try:
            ...
            while total_train_iters < train_iters:
                for j, (feed_dict, _) in enumerate(self.train_minibatches()):
                    ...
                    start_time = time.time()
                    _, loss, accuracy, n_correct, sent_num = sess.run(self.ops['train_op'], feed_dict=feed_dict)
                    train_time += time.time() - start_time
                    train_loss += loss
                    n_train_sents += len(train_targets)
                    n_train_correct += n_correct
                    n_train_tokens += sent_num
                    n_train_iters += 1
                    total_train_iters += 1
                    self.history['train_loss'].append(loss)
                    self.history['train_accuracy'].append(100 * n_correct / sent_num)
                    if total_train_iters == 1 or total_train_iters % validate_every == 0:
                        ...
                        with open(os.path.join(self.save_dir, 'sanitycheck.txt'), 'w') as f:
                            for k, (feed_dict, _) in enumerate(self.valid_minibatches()):
                                ...
                                start_time = time.time()
                                loss, n_correct, n_tokens, accuracy, predictions = sess.run(self.ops['valid_op'],
                                                                                  feed_dict=feed_dict)
            ...
            try:
                ...
                print('\r', end='')
                sys.exit(0)
        saver.save(sess, os.path.join(self.save_dir, self.name.lower() + '-trained'), latest_filename=self.name.lower(),
...
#            f.write(s)
        return
    def getMod(self, flag,targetlabel, mod):
        if flag==0 and targetlabel==1:
          return 'b-'+mod
        elif flag==1 and targetlabel==1:
          return 'i-'+mod
        else:
          return 'o'
    # =============================================================
...
#        training = tf.train.MomentumOptimizer(0.01, 0.9)
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--model', default='Attention')
